...
//					nadawanie=1;
				...
				{
					//---------------------------------------------------------------------
					//Wywo³anie rozkazu Read Coil Status (FC=01)
					//---------------------------------------------------------------------
					if(Bufor_wej[0]==ADRES_KARTY[0]				//Adres karty
							&& Bufor_wej[1]==0x01				//Kod funkcji
							&& Bufor_wej[2]==0x00				//Adres HB
							&& Bufor_wej[3]<=0x06				//Adres LB
							...
							)
					{
						...
						Bufor_wyj[1]=0x01;						//Kod funkcji
						Bufor_wyj[2]=0x01;						//Liczba bajtów rejestrów
						//Bufor_wyj[3]=0x00;					//Rejestr
						...
						for(int i=0;i<Bufor_wej[5];i++)
						{
							...
							{
								if(PORTB & _BV(0)) tmp |=_BV(i);
								else tmp  &= ~_BV(i);
							}
							else if(i+Bufor_wej[3]==0x05)
							{
								if(PORTB & _BV(1)) tmp |=_BV(i);
								else tmp  &= ~_BV(i);
							}
							else if(i+Bufor_wej[3]==0x04)
							{
								if(PORTB & _BV(2)) tmp |=_BV(i);
								else tmp  &= ~_BV(i);
							}
							else if(i+Bufor_wej[3]==0x03)
							{
								if(PORTC & _BV(3)) tmp |=_BV(i);
								else tmp  &= ~_BV(i);
							}
							else if(i+Bufor_wej[3]==0x02)
							{
								if(PORTC & _BV(0)) tmp |=_BV(i);
								else tmp  &= ~_BV(i);
							}

							else if(i+Bufor_wej[3]==0x01)
							{
								if(PORTC & _BV(1)) tmp |=_BV(i);
								else tmp  &= ~_BV(i);
							}

							else if(i+Bufor_wej[3]==0x00)
							{
								if(PORTC & _BV(2)) tmp |=_BV(i);
								else tmp  &= ~_BV(i);
							}
						}
							...
							)
					{
						...
						if(PORTB & _BV(0)) Bufor_wyj[4]|= _BV(6);
						else Bufor_wyj[4]&= ~_BV(6);

						if(PORTB & _BV(1)) Bufor_wyj[4]|= _BV(5);
						else Bufor_wyj[4]&= ~_BV(5);

						if(PORTB & _BV(2)) Bufor_wyj[4]|= _BV(4);
						else Bufor_wyj[4]&= ~_BV(4);

						if(PORTC & _BV(3)) Bufor_wyj[4]|= _BV(3);
						else Bufor_wyj[4]&= ~_BV(3);

						if(PORTC & _BV(0)) Bufor_wyj[4]|= _BV(2);
						else Bufor_wyj[4]&= ~_BV(2);

						if(PORTC & _BV(1)) Bufor_wyj[4]|= _BV(1);
						else Bufor_wyj[4]&= ~_BV(1);

						if(PORTC & _BV(2)) Bufor_wyj[4]|= _BV(0);
					...
					//---------------------------------------------------------------------
					//Wywo³anie rozkazu Write Single Output Coil  (FC=05)
					//---------------------------------------------------------------------
					else if(Bufor_wej[0]==ADRES_KARTY[0]		//Adres karty
							&& Bufor_wej[1]==0x05				//Kod funkcji
							&& Bufor_wej[2]==0x00				//Adres HB
							&& (Bufor_wej[3]==0x00				//Adres LB
								|| Bufor_wej[3]==0x01
								|| Bufor_wej[3]==0x02
							...
							)
					{
						...
						if(Bufor_wej[3]==0x06)
						{
							if (Bufor_wej[4] == 0xFF) PORTB |= _BV(0);
							else PORTB &= ~_BV(0);
						}

						else if(Bufor_wej[3]==0x05)
						{
							if (Bufor_wej[4] == 0xFF) PORTB |= _BV(1);
							else PORTB &= ~_BV(1);
						}

						else if(Bufor_wej[3]==0x04)
						{
								if (Bufor_wej[4] == 0xFF) PORTB |= _BV(2);
								else PORTB &= ~_BV(2);
						}

						else if(Bufor_wej[3]==0x03)
						{
							if (Bufor_wej[4] == 0xFF) PORTC |= _BV(3);
							else PORTC &= ~_BV(3);
						}
						else if(Bufor_wej[3]==0x02)
						{
							if (Bufor_wej[4] == 0xFF) PORTC |= _BV(0);
							else PORTC &= ~_BV(0);
						}

						else if(Bufor_wej[3]==0x01)
						{
							if (Bufor_wej[4] == 0xFF) PORTC |= _BV(1);
							else PORTC &= ~_BV(1);
						}

						else if(Bufor_wej[3]==0x00)
						{
							if (Bufor_wej[4] == 0xFF) PORTC |= _BV(2);
							else PORTC &= ~_BV(2);
						}
						//-----------------------------------------------
						...
						if(Bufor_wej[3]==0x06)
						{
							if(PORTB & _BV(0)) Bufor_wyj[4] = 0xFF;
							else Bufor_wyj[4] = 0x00;
						}

						else if(Bufor_wej[3]==0x05)
						{
							if(PORTB & _BV(1)) Bufor_wyj[4] = 0xFF;
							else Bufor_wyj[4] = 0x00;
						}

						else if(Bufor_wej[3]==0x04)
						{
							if(PORTB & _BV(2)) Bufor_wyj[4] = 0xFF;
							else Bufor_wyj[4] = 0x00;
						}

						else if(Bufor_wej[3]==0x03)
						{
							if(PORTC & _BV(3)) Bufor_wyj[4] = 0xFF;
							else Bufor_wyj[4] = 0x00;
						}

						else if(Bufor_wej[3]==0x02)
						{
							if(PORTC & _BV(0)) Bufor_wyj[4] = 0xFF;
							else Bufor_wyj[4] = 0x00;
						}

						else if(Bufor_wej[3]==0x01)
						{
							if(PORTC & _BV(1)) Bufor_wyj[4] = 0xFF;
							else Bufor_wyj[4] = 0x00;
						}

						else if(Bufor_wej[3]==0x00)
						{
							if(PORTC & _BV(2)) Bufor_wyj[4] = 0xFF;
							else Bufor_wyj[4] = 0x00;
						}
							...
							)
					{
						...
						//Zapis wartoci
						//-----------------------------------------------
						if (Bufor_wej[5] & _BV(6)) PORTB |= _BV(0);
						else PORTB &= ~_BV(0);

						if (Bufor_wej[5] & _BV(5)) PORTB |= _BV(1);
						else PORTB &= ~_BV(1);

						if (Bufor_wej[5] & _BV(4)) PORTB |= _BV(2);
						else PORTB &= ~_BV(2);

						if (Bufor_wej[5] & _BV(3)) PORTC |= _BV(3);
						else PORTC &= ~_BV(3);

						if (Bufor_wej[5] & _BV(2)) PORTC |= _BV(0);
						else PORTC &= ~_BV(0);

						if (Bufor_wej[5] & _BV(1)) PORTC |= _BV(1);
						else PORTC &= ~_BV(1);

						if (Bufor_wej[5] & _BV(0)) PORTC |= _BV(2);
						else PORTC &= ~_BV(2);
						//-----------------------------------------------
						//Odczyt zapisanych wartosci
						//-----------------------------------------------
						if(PORTB & _BV(0)) Bufor_wyj[5]|= _BV(6);
						else Bufor_wyj[5]&= ~_BV(6);

						if(PORTB & _BV(1)) Bufor_wyj[5]|= _BV(5);
						else Bufor_wyj[5]&= ~_BV(5);

						if(PORTB & _BV(2)) Bufor_wyj[5]|= _BV(4);
						else Bufor_wyj[5]&= ~_BV(4);

						if(PORTC & _BV(3)) Bufor_wyj[5]|= _BV(3);
						else Bufor_wyj[5]&= ~_BV(3);

						if(PORTC & _BV(0)) Bufor_wyj[5]|= _BV(2);
						else Bufor_wyj[5]&= ~_BV(2);

						if(PORTC & _BV(1)) Bufor_wyj[5]|= _BV(1);
						else Bufor_wyj[5]&= ~_BV(1);

						if(PORTC & _BV(2)) Bufor_wyj[5]|= _BV(0);
						else Bufor_wyj[5]&= ~_BV(0);
				...
				else
				{
					wdt_reset();
					//uint16_t crc;
					//crc=crc_calc(Bufor_wej,5);

					//Bufor_wyj[0]=(uint8_t)(crc >> 8);
					//Bufor_wyj[1]=(uint8_t)crc;
					//numer_znaku_wyj=2;
					//nadawanie=1;
				}
				odbior=0;
